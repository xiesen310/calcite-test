    ...
    public static void main(String[] args) {
        ...
        while (true) {
            try {
                ...
                System.err.println(">> 请添加业务数据--> 输入 start 开始输入数据,输入 end 结束输入数据,每行代表一条数据,不同字段使用,进行分隔");
                String s = scanner.nextLine();
                if (s.trim().equalsIgnoreCase("start")) {
                    boolean flag = true;
                    while (flag) {
                        ...
                        if (line.trim().equalsIgnoreCase("end")) {
                            ...
                            ZorkMemEnumerator.add(Arrays.asList(split));
                        }
                    }
                    System.out.println(ZorkMemEnumerator.queue.size());
                }
